...
public class Snake
{
    ...
    public boolean advancePosition()
    {
        ...
        headNode.data().getShape().setColor(Color.black);
    ...
    /**
     * removes all the shape objects from the screen
     *
     * @param gridSize
     *            size of grid
     */
    public void resetSnake(int gridSize)
    {
        Node<BodyPart> currentNode = headNode;

        if (headNode == tailNode)
        {
            mainScreen.remove(headNode.data().getShape());
        }
        else
        {
            do
            {
                mainScreen.remove(currentNode.data().getShape());
                mainScreen.remove(currentNode.next().data().getShape());
                currentNode = currentNode.next();
            }
            while (currentNode.next() != null);
        }
